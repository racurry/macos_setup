...
@test "script exists and is executable" {
  [ -f "${REPO_ROOT}/scripts/mvp_system_reqs_check.sh" ]
  [ -x "${REPO_ROOT}/scripts/mvp_system_reqs_check.sh" ]
}

@test "script outputs system requirements check heading" {
  run bash "${REPO_ROOT}/scripts/mvp_system_reqs_check.sh"
...
@test "help flag displays usage information" {
  run bash "${REPO_ROOT}/scripts/mvp_system_reqs_check.sh" --help
  [ "$status" -eq 0 ]
  [[ "$output" =~ "Usage:" ]]
  [[ "$output" =~ "--unattended" ]]
}
...
@test "--unattended flag is accepted" {
  cd "${REPO_ROOT}"
  run bash "${REPO_ROOT}/scripts/mvp_system_reqs_check.sh" --unattended
