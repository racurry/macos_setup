...
OPTIONS:
  ...
  Symlink:
    --symlink       Create ~/iCloud symlink (default if no options given)

  Diagnostics:
    --diagnose      Run all iCloud Drive diagnostics
...
DESCRIPTION:
  This script manages your iCloud Drive setup:

  Symlink mode (default):
    Creates a convenient symlink from ~/iCloud to the actual iCloud Drive
    directory at ~/Library/Mobile Documents/com~apple~CloudDocs. If iCloud
    Drive is not available, the script will skip the operation.

  Diagnostic mode:
    Helps troubleshoot iCloud Drive sync problems by:
